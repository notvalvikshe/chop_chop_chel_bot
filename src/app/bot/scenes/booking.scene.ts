...
export class BookingScene {
	...
	private async showServices(@Ctx() ctx: MyContext): Promise<void> {
		try {
			...
			if (services.length === 0) {
				await ctx.reply('К сожалению, нет доступных услуг.', mainMenuKeyboard());
			...
			const buttons = services.map((service: Service) => {
				...
				const priceRange = service.price_min === service.price_max ? `${service.price_min}₽` : `${service.price_min}-${service.price_max}₽`;
				return [Markup.button.callback(`${service.title} (${priceRange}, ${duration}мин)`, `service_${service.id}`)];
			});
			...
			await ctx.reply('Ошибка при загрузке услуг. Попробуйте позже.', mainMenuKeyboard());
			await ctx.scene.leave();
		}
	}
	...
	private async showStaff(@Ctx() ctx: MyContext, serviceId: number): Promise<void> {
		try {
			const staff = await this.bookingService.getStaffForService(serviceId);

			if (staff.length === 0) {
				await ctx.reply('К сожалению, нет свободных мастеров для этой услуги.', mainMenuKeyboard());
				await ctx.scene.leave();
				return;
			}
			...
			const buttons = [
				...
				...staff.map((s: Staff) => [Markup.button.callback(`👨‍💼 ${s.name}`, `staff_${s.id}`)]),
				[Markup.button.callback('⬅️ Назад', 'back_to_services'), Markup.button.callback('❌ Отмена', 'cancel')],
			];

			await ctx.editMessageText('👨‍💼 Выберите мастера:', Markup.inlineKeyboard(buttons));
		} catch (error) {
			this.logger.error('Failed to load staff', error);
			await ctx.reply('Ошибка при загрузке мастеров. Попробуйте позже.', mainMenuKeyboard());
			await ctx.scene.leave();
		}
	}
	...
	async onStaffSelected(@Ctx() ctx: MyContext): Promise<void> {
		...
		const staffId = staffData === 'any' ? undefined : Number.parseInt(staffData);
		ctx.scene.session.booking.staffId = staffId;

		if (staffId) {
			const selectedStaff = ctx.scene.session.staff?.find((s: Staff) => s.id === staffId);
			if (selectedStaff) {
	...
	private async showDates(@Ctx() ctx: MyContext): Promise<void> {
		...
		buttons.push([Markup.button.callback('⬅️ Назад', 'back_to_staff'), Markup.button.callback('❌ Отмена', 'cancel')]);
	...
	async onDateSelected(@Ctx() ctx: MyContext): Promise<void> {
		...
		await ctx.answerCbQuery(`Выбрана дата: ${new Date(dateStr).toLocaleDateString('ru-RU')}`);
	...
	private async showTimes(@Ctx() ctx: MyContext, date: string): Promise<void> {
		try {
			...
			const times = await this.bookingService.getAvailableSlots(serviceId, staffId, date);

			if (times.length === 0) {
				await ctx.editMessageText(
					'К сожалению, на эту дату нет свободных слотов.',
					Markup.inlineKeyboard([
						[Markup.button.callback('⬅️ Назад к датам', 'back_to_dates'), Markup.button.callback('❌ Отмена', 'cancel')],
					]),
				);
				return;
			}
			...
			buttons.push([Markup.button.callback('⬅️ Назад', 'back_to_dates'), Markup.button.callback('❌ Отмена', 'cancel')]);

			await ctx.editMessageText('⏰ Выберите время:', Markup.inlineKeyboard(buttons));
		} catch (error) {
			this.logger.error('Failed to load time slots', error);
			await ctx.reply('Ошибка при загрузке свободных слотов.', mainMenuKeyboard());
			await ctx.scene.leave();
		}
	}
	...
	async onText(@Ctx() ctx: MyContext): Promise<void> {
		...
		if (ctx.scene.session.booking.awaitingPhone) {
			...
			if (!match) {
				await ctx.reply('❌ Неверный формат телефона. Введите номер в формате: +79991234567 или 89991234567');
				return;
			}
			...
			if (!ctx.user.yclientsEmail) {
				...
				const updatedUser = await this.userRepository.updateContactInfo(ctx.user.id, phone, ctx.user.yclientsEmail);
				if (updatedUser) {
	...
	private async showConfirmation(@Ctx() ctx: MyContext): Promise<void> {
		...
		const selectedService = ctx.scene.session.services?.find((s: Service) => s.id === serviceId);
		const selectedStaff = staffId ? ctx.scene.session.staff?.find((s: Staff) => s.id === staffId) : null;
		...
		const buttons = [[Markup.button.callback('✅ Подтвердить', 'confirm'), Markup.button.callback('❌ Отмена', 'cancel')]];
	...
	async onConfirm(@Ctx() ctx: MyContext): Promise<void> {
		...
		try {
			...
			// Создаем запись
			const record = await this.bookingService.createBooking(serviceId, staffId, datetime, ctx.user);
			...
			await ctx.reply('Произошла ошибка при создании записи. Попробуйте позже.', mainMenuKeyboard());
			await ctx.scene.leave();
		}
	}
	...
	private async showServicesInfo(@Ctx() ctx: MyContext): Promise<void> {
		try {
			...
			if (services.length === 0) {
				await ctx.reply('К сожалению, пока нет доступных услуг.', mainMenuKeyboard());
				return;
			}
			...
			await ctx.reply('Произошла ошибка при загрузке услуг. Попробуйте позже.', mainMenuKeyboard());
		}
	}
