...
export class UserRepository {
  ...
  public async findWithFilters(
    ...
    if (filters.yclientsPhone) {
      conditions.push(
        ilike(userSchema.yclientsPhone, `%${filters.yclientsPhone}%`),
      );
    }

    if (filters.yclientsEmail) {
      conditions.push(
        ilike(userSchema.yclientsEmail, `%${filters.yclientsEmail}%`),
      );
    }
    ...
    if (filters.createdFrom) {
      conditions.push(
        gte(
          ...
          filters.createdFrom.toISOString().split("T")[0],
        ),
      );
    }
    ...
    if (filters.createdTo) {
      conditions.push(
        lte(
          userSchema.createdAt,
          filters.createdTo.toISOString().split("T")[0],
        ),
      );
    }
