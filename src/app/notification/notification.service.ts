...
export class NotificationService {
	...
	private scheduledMessages: Map<string, { message: ScheduledMessage; timeoutId: NodeJS.Timeout }> = new Map();
	 ...
	 */
	async sendMessage(options: SendMessageOptions): Promise<SendMessageResult | ScheduledMessage> {
	 ...
	 */
	async sendImmediate(userIds: number[], message: string, parseMode?: 'Markdown' | 'MarkdownV2' | 'HTML'): Promise<SendMessageResult> {
			...
			await Promise.all(
				batch.map(async (user) => {
					try {
						...
						result.sent++;
						this.logger.debug(`Message sent to user ${user.id} (telegram: ${user.telegramId})`);
					} catch (error) {
						result.failed++;
						...
						this.logger.error(`Failed to send message to user ${user.id}: ${errorMessage}`);
					}
				}),
			);
		...
		this.logger.log(`Broadcast complete: ${result.sent} sent, ${result.failed} failed`);
		return result;
	}
	 ...
	 */
	scheduleMessage(userIds: number[], message: string, scheduledAt: Date, parseMode?: 'Markdown' | 'MarkdownV2' | 'HTML'): ScheduledMessage {
		...
		this.logger.log(`Message scheduled: ${id}, will be sent at ${scheduledAt.toISOString()}`);
	...
	async broadcastToWhitelist(
		...
		const { users } = await this.userRepository.findWithFilters({ inWhitelist: true });
