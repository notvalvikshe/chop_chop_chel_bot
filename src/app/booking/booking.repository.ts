...
export class BookingRepository {
  ...
  async getActiveBookings(userId: number): Promise<Booking[]> {
    ...
    const bookings = await this.db.db
      ...
      .where(
        and(
          eq(bookingSchema.userId, userId),
          eq(bookingSchema.status, "active"),
          gte(bookingSchema.datetime, now),
        ),
      )
   ...
   */
  async findByYclientsRecordId(
    yclientsRecordId: number,
  ): Promise<Booking | null> {
    const [booking] = await this.db.db
      .select()
      .from(bookingSchema)
      .where(eq(bookingSchema.yclientsRecordId, yclientsRecordId))
      .limit(1);
