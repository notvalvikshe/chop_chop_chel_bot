...
export class BookingService {
	...
	async getAvailableServices(): Promise<Service[]> {
		...
		this.logger.debug(`Services data: ${JSON.stringify(allServices.map((s) => ({ id: s.id, title: s.title, is_online: s.is_online })))}`);
	...
	async getServicesByCategory(categoryId: number): Promise<Service[]> {
		const allServices = await this.yclientsApi.listServices(this.companyId, categoryId);
		return allServices.filter((service) => service.is_online);
	}
	...
	async getStaffForService(serviceId: number): Promise<Staff[]> {
		const allStaff = await this.yclientsApi.listStaff(this.companyId, [serviceId]);
	 ...
	 */
	async getAvailableSlots(serviceId: number, staffId: number | undefined, date: string): Promise<TimeSlot[]> {
		if (!staffId) {
			return [];
		}
	 ...
	 * Перенести запись
	 */
	async rescheduleBooking(recordId: number, newDatetime: string, userToken: string): Promise<Appointment> {
