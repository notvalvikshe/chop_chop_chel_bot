...
export class YClientsApiService {
	...
	async startAuth(phone: string): Promise<AuthStartResponse> {
		const response = await this.httpClient.post<YClientsApiResponse<AuthStartResponse>>(
			'/api/v1/auth',
			{ login: phone },
			{
	 ...
	 */
	async confirmAuth(phone: string, code: string, uuid: string): Promise<AuthConfirmResponse> {
		const response = await this.httpClient.post<YClientsApiResponse<AuthConfirmResponse>>(
			'/api/v1/auth',
	...
	async getBookFormConfig(bookFormId: number): Promise<BookFormConfig> {
		const response = await this.httpClient.get<YClientsApiResponse<BookFormConfig>>(`/api/v1/bookform/${bookFormId}`, {
	...
	async getCompany(companyId: number): Promise<Company> {
		const response = await this.httpClient.get<YClientsApiResponse<Company>>(`/api/v1/companies/${companyId}`, {
			headers: { Authorization: this.getAuthHeader() },
		});
		return response.data.data;
	}
	...
	async listServiceCategories(companyId: number): Promise<ServiceCategory[]> {
		const response = await this.httpClient.get<YClientsApiResponse<ServiceCategory[]>>(`/api/v1/company/${companyId}/service_categories`, {
			headers: { Authorization: this.getAuthHeader(this.userToken) },
		});
		return response.data.data;
	}
	 ...
	 */
	async listServices(companyId: number, categoryId?: number): Promise<Service[]> {
		const params: Record<string, unknown> = {};
		if (categoryId !== undefined) {
			params.category_id = categoryId;
		}

		const response = await this.httpClient.get<YClientsApiResponse<Service[]>>(`/api/v1/company/${companyId}/services`, {
			headers: { Authorization: this.getAuthHeader(this.userToken) },
			params,
		});
		return response.data.data;
	...
	async listStaff(companyId: number, serviceIds?: number[]): Promise<Staff[]> {
		...
		const response = await this.httpClient.get<YClientsApiResponse<Staff[]>>(`/api/v1/company/${companyId}/staff`, {
			headers: { Authorization: this.getAuthHeader(this.userToken) },
			params,
		});
		return response.data.data;
	 ...
	 */
	async getFreeTimes(staffId: number, date: string, serviceIds: number[]): Promise<TimeSlot[]> {
		const params: Record<string, string> = {};
		serviceIds.forEach((id, index) => {
			params[`service_ids[${index}]`] = String(id);
		});

		const response = await this.httpClient.get<YClientsApiResponse<TimeSlot[]>>(`/api/v1/book_times/${this.companyId}/${staffId}/${date}`, {
			headers: { Authorization: this.getAuthHeader(this.userToken) },
			params,
		});
		return response.data.data;
	 ...
	 */
	async getUserRecords(params: GetUserRecordsParams, userToken?: string): Promise<Appointment[]> {
		const response = await this.httpClient.get<YClientsApiResponse<Appointment[]>>('/api/v1/user/records', {
			headers: {
				Authorization: this.getAuthHeader(userToken || this.userToken),
			},
			params,
	 ...
	 */
	async rescheduleRecord(recordId: number, datetime: string, userToken?: string): Promise<Appointment> {
		const response = await this.httpClient.put<YClientsApiResponse<Appointment>>(
